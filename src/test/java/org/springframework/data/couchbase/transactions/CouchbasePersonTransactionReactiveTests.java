...
@SpringJUnitConfig(CouchbasePersonTransactionReactiveTests.Config.class)
@Transactional(transactionManager = BeanNames.COUCHBASE_TRANSACTION_MANAGER)
public class CouchbasePersonTransactionReactiveTests extends JavaIntegrationTests {
	...
	public static void beforeAll() {
		...
		// short names
		cName = null;// cName;
		// context = new AnnotationConfigApplicationContext(CouchbasePersonTransactionReactiveTests.Config.class,
		// PersonService.class);
	}
	...
	public void beforeEachTest() {
		...
		// operations = context.getBean(ReactiveCouchbaseTemplate.class);
		...
		// new Person(null, "Walter", "White")
		Person pp = repo.withScope("_default").findByFirstname("Walter");
	}
	...
	public void afterTransaction() {
		...
		assertionList.forEach(it -> {
			...
			Person p =  (Person)(operations.findById(it.getPersistable().getClass()).one(it.getId().toString()).block());
	...
	public void shouldRollbackAfterExceptionOfTxAnnotatedMethod() {
		...
		try {
			...
			if (e instanceof PoofException || (e.getMessage() != null && e.getMessage().contains("poof"))) {
		...
		assertAfterTransaction(p).isNotPresent();
	...
	*/
	...
	static class PoofException extends RuntimeException {
		public PoofException(String s) {
			super(s);
		}
	};

	static void throwPoofException() {
		throw new PoofException("poof!");
	}
