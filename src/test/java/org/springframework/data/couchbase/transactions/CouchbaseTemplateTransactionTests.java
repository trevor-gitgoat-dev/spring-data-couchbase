...
@SpringJUnitConfig(CouchbaseTemplateTransactionTests.Config.class)
public class CouchbaseTemplateTransactionTests extends JavaIntegrationTests {
	...
	public void verifyDbState() {
		...
		//Collection collection =  template.getCollection("_default") ;// client.getDatabase(DB_NAME).getCollection(COLLECTION_NAME);
		...
		assertionList.forEach(it -> {
			...
			boolean isPresent = template.findById(Assassin.class).one(it.getId().toString()) != null; // (Filters.eq("_id", it.getId())) != 0;
